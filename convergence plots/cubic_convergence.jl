...
a = -1/3 * 1
CubicPhase = PolynomialPhase(-im*[0,-3a,0,1])
# CubicPhase = PolynomialPhase([3,5,6,2,9,5,1,4,1,3])
# CubicPhase = PolynomialPhase(P.coeffs)
# CubicPhase = PolynomialPhase([0,1,2,3,4,5,6] * 0.1)
# x,y = 1,2
# CubicPhase = PolynomialPhase([0, x, y, 0, 1])

# P = 6
# r = 0.1
# CubicPhase = PolynomialPhase([0.0; -r^P; zeros(P-1); 1.0/(P+1)])

# CubicPhase = PolynomialPhase([0, -8+24im, -(10+44im)/3, -(4-28im)/9, (1+im), -4/15, 1/18])

val,fig = nsd([-1,1],z->1.0,CubicPhase,100; plot_sd = true)
@show val
fig[1]
...
for (i,ω) in enumerate(freqs)
    ref = PathFinder.nsd([-π/3,π/3],f,CubicPhase,ω; N = 50, infcontour = [true, true])
    for (n,N) in enumerate(nquads)
        val = PathFinder.nsd([-π/3,π/3],f,CubicPhase,ω; N, infcontour = [true true], Cball = 8π/8)
...
for nquad in [5,10,15,20,30]
    for (i,x) in enumerate(xArray)
        ...
        integral = PathFinder.nsd(-π/3, π/3, x -> 1.0, G, 1.0; N=nquad, infcontour=[true,true])
