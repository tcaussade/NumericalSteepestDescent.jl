...
function integrate(γ::ComplexContour, f, G, ω, x, w)
    if contour_type(γ) == :finite   
        return integrate_finite(γ, f, G, ω, x, w)
    elseif contour_type(γ) == :infiniteSD
        return integrate_infiniteSD(γ, f, G, ω, x, w)
    elseif contour_type(γ) == :finiteSD
        return integrate_finiteSD(γ, f, G, ω, x, w)
    end
end
...
function integrate_infiniteSD(γ::ComplexContour, f::Function, G::AbstractPhaseFunction, ω, x, w)
...
function complexified_nodes(η, g, dg, xvec::Vector)
    ...
    for j in 2:length(xvec)
        h[j] = Roots.newton(u -> g(u) - g(η) - im * xvec[j], dg, h[j-1]) # x0 = h[j-1] 
    end
    return h
end
