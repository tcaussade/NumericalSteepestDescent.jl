...
example1 = ([0,+0.05,0,1])
example1 = [0,0,1]
PolyPhase = PolynomialPhase(example1) 
val, figs = nsd([-1,+1im, 1],f,PolyPhase,ω; plot_graph = true, plot_sd = true)
figs[1]
figs[2]


_,fig = nsd(π, 0.0, x -> 1.0, PolynomialPhase([0, -1, +0.0, 0, 1]), 1.0, infcontour=[true,true],
plot_sd = true, Nrays = 5) # should take ~0.3ms with @benchmark 

val = nsd([-1,1],f,PolyPhase,ω)
...
val0, figs = nsd([0.0, 1.0],f,SqrtPhase,ω; quadtype = :gaussian, N=10, 
